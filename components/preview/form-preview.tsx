...
function PreviewBlock({ 
  ...
  if (block.type === "link_preview") {
    return (
      <div className="space-y-1">
        ...
        <div className="p-3 border border-dashed border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-400 flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
          Link preview — paste URLs in the live experience
        </div>
      </div>
    );
  }
  ...
  if (block.type === "book_call") {
    return (
      <div className="p-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 text-center space-y-2">
        {block.title && <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{block.title}</div>}
        {block.text && <div className="text-xs text-zinc-500">{block.text}</div>}
        <button
          type="button"
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg"
          onClick={() => block.bookingUrl && window.open(block.bookingUrl, "_blank")}
        >
